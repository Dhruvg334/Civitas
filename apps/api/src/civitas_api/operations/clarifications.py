...
def ask_clarifications(
    ...
    if current_status not in {"awaiting_clarification"}:
        # Allow any non-terminal state to ask. New clarifications during
        # 'awaiting_review' or later are uncommon but the contract doesn't
        # forbid them — ref §3 keeps 'awaiting_clarification' as an explicit
        # state so we transition explicitly.
