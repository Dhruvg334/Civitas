...
def list_incidents(
    ...
    rows = reports_ops.list_incidents(
        status=status, category=category, since=since, limit=limit
    )
    out = []
    for r in rows:
...
def merge_incident(
    ...
    return success_envelope({
        ...
        "source": source,
        "confidence": confidence,
        "created_at": inserted["created_at"].isoformat() if hasattr(inserted["created_at"], "isoformat") else str(inserted["created_at"]),
        "trace_id": trace_id,
