...
from civitas_api.core.spatial import DEFAULT_BOUNDARY, GeoPoint, get_candidate_retriever, get_landmark_index
...
def nearby_candidates(body: NearbyCandidatesRequest, _: Annotated[None, Depends(require_internal_key)]) -> dict[str, Any]:
    ...
    result=retriever.retrieve(spec, memory_incidents=memory_incidents, boundary=DEFAULT_BOUNDARY, now=body.submitted_at)
    candidates=[]
...
def media_metadata(media_id: str, _: Annotated[None, Depends(require_internal_key)]) -> dict[str, Any]:
