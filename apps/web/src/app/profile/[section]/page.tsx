...
const pages: Record<string, { title: string; intro: string; items: Array<[string, string]> }> = {
  ...
  security: { title: "Security & account", intro: "Civitas authentication is secured by cryptographic session tokens, Supabase identity verification, and role-governed authorization.", items: [["Password reset", "Email verification is required before password resets are processed."], ["Municipal access", "Operational review is role-controlled (citizen, triage, supervisor, reviewer, admin) and is never implied by a resident account."], ["Session management", "Sessions are securely persisted and verified via the Civitas /api/v1/me endpoint."]] },
};
