"""DuplicateDetector: end-to-end duplicate intelligence.
...
3. pairwise scoring: text + image + GPS + time + category + landmarks,
4. composite decision per pair (explainable `DuplicateResult`),
5. connected-component clustering into incident clusters.
"""
...
class DuplicateDetector:
    ...
    def pair_features(self, a: ReportLike, b: ReportLike, text_cache: dict[str, list[float]]) -> PairFeatures:
        ...
        cat_agr = category_agreement(a.category, b.category)
    ...
    def evaluate_pair(self, a: ReportLike, b: ReportLike) -> DuplicateResult:
        ...
        features = self.pair_features(a, b, text_cache)
    ...
    def cluster(
        ...
        """
        if len(reports) < 2:
            return cluster_reports(reports, [], self.config)
        ...
        clusters = cluster_reports(reports, scored, self.config)
        return clusters
